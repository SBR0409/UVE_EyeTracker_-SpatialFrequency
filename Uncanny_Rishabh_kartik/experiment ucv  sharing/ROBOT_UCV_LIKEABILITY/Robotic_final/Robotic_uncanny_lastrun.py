...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='E:\\Visual_Cognition\\Uncanny_Rishabh_kartik\\experiment ucv  sharing\\ROBOT_UCV_LIKEABILITY\\Robotic_final\\Robotic_uncanny_lastrun.py',
...
# --- Initialize components for Routine "Likability" ---
...
ratingText_3 = visual.TextStim(win=win, name='ratingText_3',
    ...
    languageStyle='LTR',
    depth=-2.0);
instruct_Likeability = visual.TextStim(win=win, name='instruct_Likeability',
...
for thisRobotic_trial in robotic_trials:
    ...
    # --- Prepare to start Routine "Likability" ---
    continueRoutine = True
    routineForceEnded = False
    # update component parameters for each repeat
    ...
    # keep track of which components have finished
    LikabilityComponents = [Likebility_slider_3, ratingText_3, instruct_Likeability, image_3, mouse_3]
    for thisComponent in LikabilityComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        thisPos_3 = Likebility_slider_3.getMarkerPos()
        formatted_thisPos_3=np.round(thisPos_3,1)
        ...
        if ratingText_3.status == STARTED:  # only update if drawing
            ratingText_3.setText('Current rating : ' + str(formatted_thisPos_3), log=False)
        
        # *instruct_Likeability* updates
        if instruct_Likeability.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            instruct_Likeability.frameNStart = frameN  # exact frame index
            instruct_Likeability.tStart = t  # local t and not account for scr refresh
            instruct_Likeability.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instruct_Likeability, 'tStartRefresh')  # time at next scr refresh
            instruct_Likeability.setAutoDraw(True)
        
        # *image_3* updates
        if image_3.status == NOT_STARTED and Likebility_slider_3.rating:
            # keep track of start time/frame for later
        ...
        # *mouse_3* updates
        if mouse_3.status == NOT_STARTED and Likebility_slider_3.rating:
            # keep track of start time/frame for later
    ...
    # --- Ending Routine "Likability" ---
    for thisComponent in LikabilityComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    robotic_trials.addData('Likebility_slider_3.response', Likebility_slider_3.getRating())
    robotic_trials.addData('Likebility_slider_3.rt', Likebility_slider_3.getRT())
    # store data for robotic_trials (TrialHandler)
    # the Routine "Likability" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
