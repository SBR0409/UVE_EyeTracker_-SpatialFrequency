...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='E:\\Visual_Cognition\\Uncanny_Rishabh_kartik\\experiment ucv  sharing\\HUMAN_UCV_LIKEABILITY\\Human uncanny\\Human_uncanny_lastrun.py',
...
Likebility_slider_3 = visual.Slider(win=win, name='Likebility_slider_3',
    startValue=0.00, size=(1.0, 0.1), pos=(0, -0.1), units=None,
    labels=["Least","Most"], ticks=(-100, 100), granularity=0.0,
...
instruct_Likeability_2 = visual.TextStim(win=win, name='instruct_Likeability_2',
    text='Rate the Likeability',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
for thisHuman_trial in human_trials:
    ...
    # keep track of which components have finished
    LikeabilityComponents = [Likebility_slider_3, ratingText, image, mouse, instruct_Likeability_2]
    for thisComponent in LikeabilityComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    while continueRoutine:
        ...
        thisPos_4 = Likebility_slider_3.getMarkerPos()
        formatted_thisPos_4=np.round(thisPos_4,1)
        
        #formatted_thisPos=str.format('{0:.3f}', thisPos)
        
        
        # *Likebility_slider_3* updates
        if Likebility_slider_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            Likebility_slider_3.frameNStart = frameN  # exact frame index
            Likebility_slider_3.tStart = t  # local t and not account for scr refresh
            Likebility_slider_3.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(Likebility_slider_3, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'Likebility_slider_3.started')
        ...
        if ratingText.status == STARTED:  # only update if drawing
            ratingText.setText('Current rating : ' + str(formatted_thisPos_4), log=False)
        
        # *image* updates
        if image.status == NOT_STARTED and Likebility_slider_3.rating:
            # keep track of start time/frame for later
        ...
        # *mouse* updates
        if mouse.status == NOT_STARTED and Likebility_slider_3.rating:
            # keep track of start time/frame for later
        ...
        # *instruct_Likeability_2* updates
        if instruct_Likeability_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            instruct_Likeability_2.frameNStart = frameN  # exact frame index
            instruct_Likeability_2.tStart = t  # local t and not account for scr refresh
            instruct_Likeability_2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instruct_Likeability_2, 'tStartRefresh')  # time at next scr refresh
            instruct_Likeability_2.setAutoDraw(True)
    ...
    for thisComponent in LikeabilityComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    human_trials.addData('Likebility_slider_3.response', Likebility_slider_3.getRating())
    human_trials.addData('Likebility_slider_3.rt', Likebility_slider_3.getRT())
    # store data for human_trials (TrialHandler)
    human_trials.addData('mouse.x', mouse.x)
    human_trials.addData('mouse.y', mouse.y)
    human_trials.addData('mouse.leftButton', mouse.leftButton)
    human_trials.addData('mouse.midButton', mouse.midButton)
    human_trials.addData('mouse.rightButton', mouse.rightButton)
    human_trials.addData('mouse.time', mouse.time)
    human_trials.addData('mouse.clicked_name', mouse.clicked_name)
