...
function recordingRoutineBegin(snapshot) {
  return async function () {
    ...
    recordingComponents.forEach( function(thisComponent) {
...
function Human_StimuliRoutineBegin(snapshot) {
  return async function () {
    ...
    Human_StimuliComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function Mechano_Human_likenessRoutineBegin(snapshot) {
  return async function () {
    ...
    Mechano_Human_likenessComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function LikeabilityRoutineBegin(snapshot) {
  return async function () {
    ...
    LikeabilityComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
