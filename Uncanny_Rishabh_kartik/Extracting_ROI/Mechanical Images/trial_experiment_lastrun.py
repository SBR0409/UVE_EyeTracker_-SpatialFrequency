...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\karti\\OneDrive\\Desktop\\Mechanical Images\\trial_experiment_lastrun.py',
...
eyes = visual.Rect(
    win=win, name='eyes',
    width=(0.34, 0.16)[0], height=(0.34, 0.16)[1],
    ori=0.0, pos=(0, 0.02), anchor='center',
    lineWidth=1.0,     colorSpace='rgb',  lineColor=[-1.0000, -1.0000, 1.0000], fillColor=[0.0000, 0.0000, 0.0000],
    opacity=0.5, depth=-2.0, interpolate=True)
mouth = visual.Rect(
    win=win, name='mouth',
    width=(0.29, 0.11)[0], height=(0.29, 0.11)[1],
    ori=0.0, pos=(0, -0.12), anchor='center',
...
trials = data.TrialHandler(nReps=1.0, method='random', 
    ...
    trialList=data.importConditions('robotic_images.xlsx'),
...
for thisTrial in trials:
    ...
    trialComponents = [whole_image, image, eyes, mouth]
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        if whole_image.status == STARTED:
            ...
            if tThisFlipGlobal > whole_image.tStartRefresh + 3.0-frameTolerance:
        ...
        if eyes.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > eyes.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if mouth.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > mouth.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
