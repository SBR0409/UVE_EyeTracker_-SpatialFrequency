...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\karti\\OneDrive\\Desktop\\Humanoid images\\trial_experiment_lastrun.py',
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() < 10.0:
        ...
        if whole_image.status == STARTED:
            ...
            if tThisFlipGlobal > whole_image.tStartRefresh + 10.0-frameTolerance:
        ...
        if forehead.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > forehead.tStartRefresh + 10.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if eyes.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > eyes.tStartRefresh + 10.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if nose.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > nose.tStartRefresh + 10.0-frameTolerance:
                # keep track of stop time/frame for later
        ...
        if mouth.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > mouth.tStartRefresh + 10.0-frameTolerance:
                # keep track of stop time/frame for later
